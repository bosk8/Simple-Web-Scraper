...
public class HttpFetcher {
  ...
  /**
   * Constructs a new HttpFetcher with default settings.
   */
  public HttpFetcher() {
  ...
  /**
   * Constructs a new HttpFetcher with custom settings.
   *
    ...
    return CompletableFuture.supplyAsync(
        () -> {
          try {
            ...
            Semaphore semaphore =
                domainSemaphores.computeIfAbsent(
                    domain, k -> new Semaphore(MAX_CONCURRENT_REQUESTS_PER_DOMAIN));
      ...
      try {
        ...
        if (statusCode >= 200 && statusCode < 300) {
          ...
          if (attempt < maxRetries) {
            ...
            logger.warn(
                "Server error for URL: {} (status: {}), retrying in {}ms (attempt {}/{})",
                url,
                statusCode,
                delayMs,
                attempt + 1,
        ...
        if (attempt < maxRetries) {
          ...
          logger.warn(
              "Exception fetching URL: {}, retrying in {}ms (attempt {}/{})",
              url,
              delayMs,
              attempt + 1,
              maxRetries,
