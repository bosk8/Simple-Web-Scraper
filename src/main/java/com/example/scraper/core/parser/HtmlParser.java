...
public class HtmlParser {
   ...
   */
  public List<ScrapedData> parseHtml(
      String htmlContent, String baseUrl, DataSelectors selectors) {
    try {
  ...
  private List<ScrapedData> extractData(Document document, DataSelectors selectors) {
    ...
    try {
      ...
      logger.debug(
          ...
          containers.size(),
          selectors.getContainerSelector());
  ...
  private ScrapedData extractDataFromContainer(Element container, DataSelectors selectors) {
    ...
    extractText(container, selectors.getTitleSelector())
        .ifPresent(title -> data.setTitle(normalizeText(title)));
    extractText(container, selectors.getDescriptionSelector())
        .ifPresent(desc -> data.setDescription(normalizeText(desc)));
    extractUrl(container, selectors.getUrlSelector())
        .ifPresent(url -> data.setUrl(normalizeUrl(url)));

    if (selectors.getPriceSelector() != null) {
      extractText(container, selectors.getPriceSelector())
          .ifPresent(price -> data.setPrice(normalizeText(price)));
    }
    if (selectors.getImageSelector() != null) {
      extractUrl(container, selectors.getImageSelector())
          .ifPresent(img -> data.setImageUrl(normalizeUrl(img)));
    }
