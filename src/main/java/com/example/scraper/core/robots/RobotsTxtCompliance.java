...
public class RobotsTxtCompliance {
  ...
  private static final String USER_AGENT =
      "SimpleWebScraper/1.0 (+https://github.com/example/simple-web-scraper)";
  ...
  /**
   * Constructs a new RobotsTxtCompliance object.
   *
   * @param httpClient The HttpClient to use for fetching robots.txt files
   */
  ...
  /**
   * Gets or fetches robot rules for a domain. Rules are cached to avoid repeated requests.
   *
    ...
    try {
      ...
      if (statusCode >= 200 && statusCode < 300) {
        ...
        return parser.parseContent(
            robotsUrl, robotsContent.getBytes(StandardCharsets.UTF_8), "text/plain", USER_AGENT);
      } else if (statusCode >= 400 && statusCode < 500) {
        logger.debug(
            "robots.txt not found ({}), assuming crawling allowed for domain: {}",
            statusCode,
            domain);
        return parser.parseContent(robotsUrl, new byte[0], "text/plain", USER_AGENT);
      } else {
        logger.warn(
            "robots.txt server error ({}), blocking crawling for domain: {}", statusCode, domain);
        return parser.parseContent(
            robotsUrl,
  ...
  /**
   * Clears the robots.txt cache. Useful for testing or when rules might have changed.
   */
